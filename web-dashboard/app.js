...
(() => {
    ...
    // ── State ───────────────────────────────────────────────
    const AUTH_STORAGE_KEY = 'dombla_auth_token';

    const state = {
        authToken: localStorage.getItem(AUTH_STORAGE_KEY) || null,
        authUser: null,
    ...
    const dom = {
        ...
        btnPasswordToggle: $('btn-password-toggle'),
        eyeIcon:           $('eye-icon'),
        btnLogin:          $('btn-login'),
    ...
    async function checkAuth() {
        ...
        try {
            const resp = await fetch('/api/verify-auth', {
                headers: {
            ...
            if (resp.ok) {
                ...
                if (data.ok) {
                    setAuthenticated(true, data.user || 'dombla');
                    return true;
                }
            }
        } catch (err) {
    ...
    function setAuthenticated(isAuthed, username = 'dombla') {
        if (isAuthed) {
            ...
            if (dom.authUsername) setTimeout(() => dom.authUsername.focus(), 80);
            if (state.mqtt.ws) {
                try { state.mqtt.ws.close(); } catch (e) {}
                state.mqtt.ws = null;
            }
            stopPolling();
        }
    ...
    function showAuthError(msg) {
        ...
        if (dom.authCard) {
            ...
            void dom.authCard.offsetWidth; // trigger reflow
    ...
    function bindEvents() {
        ...
        if (dom.authForm) {
            dom.authForm.addEventListener('submit', async (e) => {
                ...
                if (dom.btnLogin) {
                    ...
                    dom.btnLogin.innerHTML = `<span>Signing in...</span>`;
                }
                hideAuthError();
                ...
                try {
                    ...
                    showAuthError('Connection failed. Please check your network.');
                } finally {
                    if (dom.btnLogin) {
                        dom.btnLogin.disabled = false;
                        dom.btnLogin.innerHTML = `<span>Log In</span>`;
                    }
                }
            });
        }
        ...
        if (dom.btnPasswordToggle && dom.authPassword) {
            dom.btnPasswordToggle.addEventListener('click', () => {
                const isPassword = dom.authPassword.type === 'password';
                dom.authPassword.type = isPassword ? 'text' : 'password';
                ...
                if (dom.eyeIcon) {
                    dom.eyeIcon.innerHTML = eyePath;
                }
                dom.btnPasswordToggle.setAttribute('aria-label', isPassword ? 'Hide password' : 'Show password');
            });
        }
    ...
    function initMQTT() {
        if (!state.authToken) {
            console.log('[MQTT Bridge] Connection deferred until authenticated');
            return;
        }

        const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
        const host = window.location.host || 'localhost:8000';
        const wsUrl = `${protocol}//${host}/ws?token=${encodeURIComponent(state.authToken)}`;
        ...
        try {
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    if (data.type === 'auth_error' || data.type === 'auth_required') {
                        console.warn('[MQTT Bridge] Auth required / rejected by server');
                        setAuthenticated(false);
                        return;
                    }
                    if (data.type === 'init') {
            ...
            ws.onclose = () => {
                ...
                if (state.authToken && !state.mqtt.reconnectTimer) {
    ...
    function initClockTick() {
        ...
        state.clockTimer = setInterval(() => {
            if (!state.clock.time) {
                // If not synced yet, periodically update UI relative to client WIB
                return;
            }
    ...
    function startAuthenticatedSession() {
        ...
        // Auto-select first device and start polling if available
        ...
        console.log('[Dombla] Authenticated session active for', state.authUser);
    }
    ...
    async function init() {
        ...
        if (authed) {
            ...
            console.log('[Dombla] Authentication required');
            updateVisibility();
        }
